...
import com.davidhenriquez.rehabilicop.listas.estado.EstadoService;
...
public class EstadoController {
	...
	public ResponseEntity<?> getEstado(UUID id){
		try {
	...
	@PreAuthorize("hasRole('crear estado')")
	public ResponseEntity<?> create(@RequestBody Estado estado) throws Exception {
		try {
	...
	@PreAuthorize("hasRole('editar estado')")
	public ResponseEntity<?> update(@RequestBody Estado estado) throws Exception {
		try {
	...
	@PreAuthorize("hasRole('eliminar estado')")
	public ResponseEntity<?> delete(@PathVariable UUID id) {
		try {
			estadoService.delete(id);
			return ResponseEntity.status(HttpStatus.OK).body(new Estado());
		} catch (ValidationException ex) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ex.getErrors());
		} catch (Exception ex) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
